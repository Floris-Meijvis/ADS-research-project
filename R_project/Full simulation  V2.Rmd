...
#Should be run N_sim times to average out RNG
N = 500
k = 4
p = 2
set.seed(seed)

beta_M = matrix(rnorm((k - 1) * p, mean = 5, sd = 5), ncol = p)
beta_signs = sample(c(-1, 1), p, replace = TRUE)
for (j in 1:p) {
  beta_M[, j] = beta_signs[j] * abs(beta_M[, j])
  }
alpha_M = rnorm(k - 1, mean = 5, sd = 5)
alpha_A = sort(runif(k - 1, -3, 3)) 
beta_A = rnorm(p, mean = 5, sd = 5)
...
Pred_class = as.factor(Estimate_OM_class(N, k, p, optimized_intercepts, optimized_coefs, as.matrix(Data_M$data[,-1])))
...
- investigate why bias sometimes increase as dataset grows larger, maybe check if this is because of Non-convergence! cannot be caused by gradient.

- is there a difference in bias between categories?
