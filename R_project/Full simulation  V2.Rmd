...
Data %>% filter(N > 30) %>%
  ...
  ylim(0,2)
```
idea: turn this into a 3d plot for a better overview
...
#Should be run N_sim times to average out RNG, and check for bugs
N = 1000
k = 4
p = 2
set.seed(seed+1)

beta_M = matrix(rnorm((k - 1) * p, mean = 3, sd = 2), ncol = p)
beta_signs = sample(c(-1, 1), p, replace = TRUE)
for (j in 1:p) {
  beta_M[, j] = beta_signs[j] * abs(beta_M[, j])
  }
alpha_M = rnorm(k - 1, mean = 1, sd = 2)
alpha_A = sort(rnorm(k - 1, mean = 1, sd = 2)) 
beta_A = rnorm(p, mean = 3, sd = 2)
