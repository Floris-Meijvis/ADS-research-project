...
write.table( t(c("N", "k", "p", "BiasMtoM", "BiasMtoA", "BiasAtoM", "BiasAtoA")), file = "Sim_Results", append = FALSE, row.names = F, col.names = F)
```
...
k = c(2,3, 4, 5)
p = c(1,2,3)
N = c(500, 1000, 2500)   #c(30, 50, 100, 250, 500, 1000, 2500, 5000, 10000)
...
SimulateAVGBias = function(row) {
  ...
  for (j in 1:Nsim) {#this subroutine performs Nsims for parameters in each row of the DataFrame
    ...
    return(c(mean(helpMtoM), mean(helpMtoA), mean(helpAtoM), mean(helpAtoA)))
}

test = apply(DataFrame, 1, SimulateAVGBias)

test = as.data.frame(t(test))
colnames(test) = c("BiasMtoM", "BiasMtoA", "BiasAtoM", "BiasAtoA")
...
Data[Data$k == 3 & Data$p == 2,] %>%
  ggplot(aes(x = N)) + 
  geom_line(aes(y = BiasMtoM, color = 'BiasMtoM')) + 
  geom_line(aes(y = BiasAtoM, color = 'BiasAtoM'))

Data[Data$k == 3 & Data$p == 2,] %>%
  ggplot(aes(x = N)) + 
  geom_line(aes(y = BiasMtoA, color = 'BiasMtoA')) + 
  geom_line(aes(y = BiasAtoA, color = 'BiasAtoA'))
```
